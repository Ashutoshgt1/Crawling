...
class Settings:
    ...
    http_concurrency: int = int(os.getenv("HTTP_CONCURRENCY", "20"))
    max_links_per_page: int = int(os.getenv("MAX_LINKS_PER_PAGE", "50"))
    lease_seconds: int = int(os.getenv("LEASE_SECONDS", "120"))
    scheduler_batch_size: int = int(os.getenv("SCHEDULER_BATCH_SIZE", "100"))
    default_crawl_delay_ms: int = int(os.getenv("DEFAULT_CRAWL_DELAY_MS", "1000"))
    default_host_parallelism: int = int(os.getenv("DEFAULT_HOST_PARALLELISM", "2"))
    kafka_enabled: bool = _bool_env("KAFKA_ENABLED", False)
